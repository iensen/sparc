...
public class WarningRuleCreator {
	...
	public static ArrayList<String> createWarningRules(
			String originalRule, int lineNumber, int columnNumber,
			ArrayList<ASTatom> bodyAtoms) {
		String ruleId ="\""+originalRule + " ( line: " + lineNumber + ", column: "
				+ columnNumber + ")\"";
		...
		for(int i=0;i<bodyAtoms.size();i++) {
			if(i!=0)
				rule1.append(",");
			rule1.append(bodyAtoms.get(i).toString());
		}
		rule1.append(".");
