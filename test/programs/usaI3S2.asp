...
stuck(rom1,closed).
stuck(rom2,closed).
goal(T_G,fwd_rcs):-h(maneuver_of(plus_x,fwd_rcs),T_G).
goal(T_G,left_rcs):-h(maneuver_of(plus_x,left_rcs),T_G).
goal(T_G,right_rcs):-h(maneuver_of(plus_x,right_rcs),T_G).
s_gen0(ffdummy).
s_gen0(lox12).
s_gen0(ffm1).
s_gen0(ffm2).
